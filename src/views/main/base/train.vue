  ...
  setup() {
    ...
    watch(() => train.value.start, ()=>{
      if (Tool.isNotEmpty(train.value.start)) {
        ...
        train.value.startPinyin = pinyin(train.value.start, { toneType: 'none'}).replace(/ /g, "");
      } else {
    ...
    watch(() => train.value.end, ()=>{
      if (Tool.isNotEmpty(train.value.end)) {
        train.value.endPinyin = pinyin(train.value.end, { toneType: 'none'}).replace(/ /g, "");
      } else {
    ...
    const genSeat = (record) => {
      loading.value = true;
      axios.get("/business/admin/train/genSeat/" + record.code).then((response) => {
        loading.value = false;
        const data = response.data;
        if (data.success) {
